...
- **`run-skill.yml`** — the only workflow with secrets. `workflow_dispatch` only;
...
1. **Environments → `ops`**: add *Required reviewers* (≥1 other maintainer) and
   *Deployment branches → Selected → `main`*. Put the secrets **here**, not at
   ...
   `GOOGLE_WORKSPACE_CLI_TOKEN` (a short-lived access token for a dedicated
   service identity that has only the Drive/Sheets access the engines need).
