...
## The 100-chapter cap and the `Status` column
...
- Unit tests for the pure logic in every engine (no network, no `gws`). This
  ...
  ```
  The four completed migrations keep their own tests beside them, and CI runs
  those too — see `references/completed-migrations.md`:
  ```bash
