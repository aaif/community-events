...
CONFIG_LABELS = {"public_prefixes": "Public channel prefix",
                 ...
                 "ops_staff_domains": "Ops staff domain"}
CONFIG_NOTES = {
    ...
    "ops_staff_domains": "An 'Ops staff email' row outside these domains is "
                         "refused, not seeded. One row per domain.",
}
...
def config_rows(cfg):
    ...
    — deliberately NOT every key in CONFIG_LABELS. `ops_staff_emails` and
    `ops_staff_domains` were never in the JSON this migration reads (both added
    2026-09-10, long after), so there is nothing here to move; their rows are
    typed onto the tab by hand. Iterating CONFIG_LABELS instead would KeyError
    on a cfg that predates them.
    """
    rows = [CONFIG_HEADERS]
